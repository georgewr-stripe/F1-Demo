...
export default function PricingTable({ prices, show, setSection }: PricingTableProps) {
  ...
  const [price, setPrice] = React.useState<number | undefined | null>(24.99);
  const [priceID, setPriceID] = React.useState<string>(prices.monthly.id);

  React.useEffect(() => {
    setPrice(pricesByCurrency[currency][annual ? "annual" : "monthly"]);
  }, [annual, currency, pricesByCurrency]);

  React.useEffect(() => {
    setPriceID(prices[annual ? "annual" : "monthly"].id);
  }, [annual, prices]);

  React.useEffect(() => {
    setCookie('subscriptionCurrency', currency);
    setCookie('subscriptionPriceID', priceID)
  }, [currency, priceID])
              ...
              <div className="mx-auto max-w-lg overflow-hidden rounded-lg shadow-lg lg:flex lg:max-w-none">
                <div className="flex-1 bg-white px-6 py-8 lg:p-12">
                  ...
                  <p className="mt-6 text-base text-gray-500">
                    Watch all the F1 sessions (free practices, qualifying and
                    the race) live or on-demand on your computer, smartphone,
                    tablet. Live streaming of F2, F3, Porsche Supercup is now
                    available
                  </p>
                  <div className="mt-8">
                ...
                <div className="bg-gray-50 pb-8 px-6 text-center lg:flex lg:flex-shrink-0 lg:flex-col lg:justify-center lg:p-12">
                  ...
                  <div className="mt-4 flex items-center justify-center text-5xl font-bold tracking-tight text-gray-900">
                    <span>{price ? formatter.format(price / 100) : ""}</span>
